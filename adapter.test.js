...
Deno.test('adapter', async (t) => {
  await t.step('createDatabase', async (t) => {
    ...
    await t.step('should 409 if database already exists', async () => {
      const db = random()
      await adapter.createDatabase(db)
  ...
  await t.step('removeDatabase', async (t) => {
    ...
    await t.step('should be empty if removed and recreated', async () => {
      const db = random()
      await adapter.createDatabase(db)
      ...
      await adapter.retrieveDocument({ db, id: 'foobar' })
        .then((res) => assertEquals(res, { ok: false, status: 404, msg: 'doc not found' }))

      // teardown
      await adapter.removeDatabase(db)
    })

    await t.step('should 404 if database does not exists', async () => {
      const db = random()
  ...
  await t.step('createDocument', async (t) => {
    await t.step('createDocument - should create the document', async () => {
      const db = random()
      await adapter.createDatabase(db)
    ...
    await t.step('should 400 if document is empty', async () => {
      const db = random()
      await adapter.createDatabase(db)
    ...
    await t.step('should 409 if document with id already exists', async () => {
      const db = random()
      await adapter.createDatabase(db)
      await adapter.createDocument({ db, id: '1234', doc: { name: 'bar' } })
  ...
  await t.step('retrieveDocument', async (t) => {
    ...
    await t.step('should 404 if document does not exist', async () => {
      const db = random()
      await adapter.createDatabase(db)
  ...
  await t.step('updateDocument', async (t) => {
    await t.step('updateDocument - should create the document', async () => {
      const db = random()
      await adapter.createDatabase(db)
  ...
  await t.step('queryDocuments', async (t) => {
    await t.step('should query the documents', async () => {
      const db = random()
      await adapter.createDatabase(db)
  ...
  await t.step('indexDocuments', async (t) => {
    await t.step('should create and name an index', async () => {
      const db = random()
      await adapter.createDatabase(db)
      await adapter.createDocument({ db, id: '5', doc: { val: 5 } })
